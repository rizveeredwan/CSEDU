...
/*For Lalr*/
int lalr_states;
int merged_nodes[MAX+1];
int lalr_parsing_table[MAX+1][MAX+1];
int type_of_operation_lalr_parsing_table[MAX+1][MAX+1];
...
/*This function checks if two states have same set of canonical covers or not*/
bool check_two_states_equality(int one, int two){
	int work_one[MAX+1],work_two[MAX+1];
	memset(work_one,0,sizeof(work_one));
	memset(work_two,0,sizeof(work_two));
	for(int i=0;i<saved_states[one].size;i++) {
		for(int j=0;j<=saved_states[two].size;j++){
			bool same = checking_equality_of_vectors(saved_states[one].canonical_set[i],saved_states[two].canonical_set[j]);
			if(same) {
				work_two[j] = 1;
				work_one[i] = 1;
			}
	...
	for(int i=0;i<saved_states[one].size;i++) {
		if(work_one[i] == 1) continue;
		else return false;
	}
	for(int i=0;i<saved_states[two].size;i++){
		if(work_two[i] == 1) continue;
		else return false; // didn't find 
	}
	return true; //same set 
}

void lalr_generate_parsing_table(){
	memset(lalr_parsing_table,-1,sizeof(lalr_parsing_table));
	memset(type_of_operation_lalr_parsing_table,-1,sizeof(type_of_operation_lalr_parsing_table));
	for(int i=1;i<=state;i++) {
		for(int j=0;j<terminal_nodes.size();j++) {
			int value = parser_table[i][terminal_nodes[j]];
			if(value == -1) continue;
			else {
				int real_node = merged_nodes[i];
				//cout<<"i = " << i << " "<< real_node << endl;
				if(lalr_parsing_table[real_node][terminal_nodes[j]] == -1) {
					//empty : no chance of error
					type_of_operation_lalr_parsing_table[real_node][terminal_nodes[j]] = type_of_operation[i][terminal_nodes[j]];
					if(type_of_operation_lalr_parsing_table[real_node][terminal_nodes[j]] == 1) {
						//reduce move
						lalr_parsing_table[real_node][terminal_nodes[j]] = value;
					}
					else {
						//shift move
						lalr_parsing_table[real_node][terminal_nodes[j]] = merged_nodes[value];
					}
				}
				else {
					//value exists
					if(type_of_operation_lalr_parsing_table[real_node][terminal_nodes[j]] != type_of_operation[i][terminal_nodes[j]]) {
						cout<<"shift reduce conflict found for state" << i << endl;
						save_state_print(i);
						exit(0);
					}
					else {
						//type are same
						if(type_of_operation[i][terminal_nodes[j]] == 1) {
							if(value != lalr_parsing_table[real_node][terminal_nodes[j]]) {
									//two different rule reduced
									cout<<"reduce reduce conflit found"<<endl;
									save_state_print(i);
									exit(0);
							}
						}
							...
							int temp = value;
							value=merged_nodes[value];
							if(value != lalr_parsing_table[real_node][terminal_nodes[j]]) {
									//two different rule reduced
									cout<<"Ambiguous Error Found"<<endl;
									save_state_print(i);
									cout<<"HHHH"<<" "<<value<<" "<<temp<<endl;
									save_state_print(value);
									cout<<"JJJ "<< merged_nodes[lalr_parsing_table[real_node][terminal_nodes[j]]]<<endl;
									save_state_print(lalr_parsing_table[real_node][terminal_nodes[j]]);
									exit(0);
							}
						}
					}
		...
		if(parser_table[i]['$'] != -1) {
			if(type_of_operation_lalr_parsing_table[merged_nodes[i]]['$'] == -1) {
					type_of_operation_lalr_parsing_table[merged_nodes[i]]['$'] = type_of_operation[i]['$'];
					if(type_of_operation[i]['$'] == 1) {
						//reduce
						lalr_parsing_table[merged_nodes[i]]['$'] = parser_table[i]['$'];
					}
					else {
						lalr_parsing_table[merged_nodes[i]]['$'] = merged_nodes[parser_table[i]['$']];
					}
			}
			else {
				//exists value
				if(type_of_operation_lalr_parsing_table[merged_nodes[i]]['$'] != type_of_operation[i]['$']) {
					cout<<"shift error conflict" << endl;
					save_state_print(i);
					exit(0);
				}
				else {
					if(type_of_operation_lalr_parsing_table[merged_nodes[i]]['$'] == 1) {
						if(parser_table[i]['$'] != lalr_parsing_table[merged_nodes[i]]['$']) {
							cout<<"reduce reduce conflit" << endl;
							exit(0);
						} 
					}
					else {
						if(merged_nodes[parser_table[i]['$']] != lalr_parsing_table[merged_nodes[i]]['$']) {
							cout<<"ambiguous"<<endl;
							exit(0);
						}
					}
		...
		for(int j=0;j<non_terminal_nodes.size();j++) {
			int value = parser_table[i][non_terminal_nodes[j]];
			if(value == -1) continue;
			else {
				int real_node = merged_nodes[i];
				if(lalr_parsing_table[real_node][non_terminal_nodes[j]] == -1) {
					//empty : no chance of error
					type_of_operation_lalr_parsing_table[real_node][non_terminal_nodes[j]] = type_of_operation[i][non_terminal_nodes[j]];
					lalr_parsing_table[real_node][non_terminal_nodes[j]] = merged_nodes[value];
				}
				else {
					//chance of shift shift conflict
					value = merged_nodes[value];
					if(value != lalr_parsing_table[real_node][non_terminal_nodes[j]]) {
						cout<<"ambiguity found"<<endl;
						save_state_print(i);
						exit(0);
					}
				}
	...
	//update now into parser table
	memset(parser_table,-1,sizeof(parser_table));
	memset(type_of_operation,-1,sizeof(type_of_operation));
	for(int i=1;i<=lalr_states;i++) {
		for(int j=0;j<non_terminal_nodes.size();j++) {
			parser_table[i][non_terminal_nodes[j]] = lalr_parsing_table[i][non_terminal_nodes[j]];
			type_of_operation[i][non_terminal_nodes[j]] = type_of_operation_lalr_parsing_table[i][non_terminal_nodes[j]]; 
		}
		for(int j=0;j<terminal_nodes.size();j++) {
			parser_table[i][terminal_nodes[j]] = lalr_parsing_table[i][terminal_nodes[j]];
			type_of_operation[i][terminal_nodes[j]] = type_of_operation_lalr_parsing_table[i][terminal_nodes[j]]; 
		}
		parser_table[i]['$'] = lalr_parsing_table[i]['$'];
		type_of_operation[i]['$'] = type_of_operation_lalr_parsing_table[i]['$']; 
...
void lalr_state_merge(){
	int worked[MAX+1];
	memset(worked,0,sizeof(worked));
	memset(merged_nodes,0,sizeof(merged_nodes));
	lalr_states = 0; 
	for(int i=1;i<=state;i++) {
		if(worked[i] == 0 && merged_nodes[i] == 0) {
			lalr_states++;
			merged_nodes[i]=lalr_states;
			for(int j=i+1;j<=state;j++) {
				if(worked[j] == 0 && merged_nodes[j] == 0) {
					//didn't check
					bool same = check_two_states_equality(i,j);
					if(same) {
						//same thing just vary in orientation of look aheads
						worked[j] = 1;
						merged_nodes[j] = merged_nodes[i];
						cout<<"same are " <<endl;
						save_state_print(i);
						cout<<"*******"<<"i " << i <<" j " << j <<" "<<merged_nodes[i] <<endl;
						save_state_print(j);
					} 
				}
			}
	...
	cout<<lalr_states<<endl;
	/*for(int i=1;i<=state;i++) {
		cout<<"i = " << i <<" "<<merged_nodes[i]<<endl;
	}*/
	return;
}
