...
void left_most_derivation(vector<info>derivation){
		...
		for(int i=0;i<temp.right_side.size();i++){
			if(M2[temp.right_side[i]] == "term2" || M2[temp.right_side[i]] == "expr2"){
				...
				for(int j=0;j<temp.right_side.size();j++){
					if(M2[temp.right_side[j]] == "epsilon") continue;
					else cout<<M2[temp.right_side[j]]<<" ";
				}
				cout<<endl;
			}
		}
...
void recursive_descent_parsing(vector<string> s,int idx,vector<info>derivation,int matched_upto){
	printf("idx = %d matched_upto %d\n",idx,matched_upto);
	for(int i=0;i<derivation.size();i++){
		info temp=derivation[i];
		for(int j=0;j<temp.right_side.size();j++){
			//printf(%s ",M2[temp.right_side[j]]);
			cout<<M2[temp.right_side[j]]<<" ";
		}
		printf("\n");
	}
	...
	if(length==0){
		...
		cout<< "node " << node << " and name " << M2[node]<<" "<<endl;
		bool terminal=true;
		for(int i=0;i<left_side[node].size();i++){ 
			terminal=false; 
			info per_rule=left_side[node][i]; //A->PQR
			printf("taken rule and boss %d\n",node);
			cout<<"boss " << M2[node]<<endl;
			for(int k=0;k<per_rule.right_side.size();k++){
				cout<<M2[per_rule.right_side[k]]<<" ";
			}
			cout<<endl;
			info sent;
			...
			printf("till now made 1\n");
			for(int k=0;k<sent.right_side.size();k++){
				cout<<M2[sent.right_side[k]]<<" ";
			}
			printf("till now made 2\n");
			for(int k=0;k<per_rule.right_side.size();k++){
...
int main(int argc, char **argv){
	...
	for(int i=1;i<=15;i++){
		...
		for(int j=0;j<left_side[i].size();j++){
			info temp=left_side[i][j];
			for(int k=0;k<temp.right_side.size();k++){
				cout<<M2[temp.right_side[k]]<<" ";
			}
			cout<<endl;
		}
	}
	input();
	/*vector<string>S;
	//number / number + id - number * (number + number)
	S.push_back("number");
	S.push_back("/");
	S.push_back("number");
	S.push_back("+");
