...
void point_rotation_2D(double x0,double y0,double angle, double &tx, double &ty){
	int dim=3;
	...
	matrix_multiplication(2,rotation_matrix,basic);
...
void draw_object(double angle,int choice){
	...
	for(int j=0;j<n;j++){
		...
		if(choice==1) {
			...
			//point_rotation_2D(in_y[j],in_z[j],angle,tx,ty);
			//point_rotation_2D(ou_y[j],ou_z[j],angle,tx1,ty1);
		}
		else if(choice==2){
			cnt++;
			if(cnt==1){
			...
			//move around z axis
			point_rotation_2D(in_x[j],in_y[j],angle,tx,ty);
			point_rotation_2D(ou_x[j],ou_y[j],angle,tx1,ty1);
		}
		x_st=tx;
		y_st=ty;
		x_en=tx1;
