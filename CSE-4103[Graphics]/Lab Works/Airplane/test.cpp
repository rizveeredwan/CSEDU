...
bool cmp_surface(Surface &a, Surface &b){
	...
	double b_min = INF;
	for(int i=0;i<a.n;i++) {
		a_min = min(min(a_min,a.line[i].z1),a.line[i].z2);
	}
	for(int i=0;i<b.n;i++) {
		b_min = min(min(b_min,b.line[i].z1),b.line[i].z2);
	}
	a_min = a_min - S.z;
	b_min = b_min - S.z;
...
void polygon_filling(Surface temp){
	...
	double shade = shading(temp);
	if(shade>=0) {
		shading_cos_value=shade;
		active_list();
	}
	/*else {
		shading_cos_value=1;
...
void controlling2(){
	...
	sort(surface,surface+connection,cmp_surface);
	//now drawing
	for(int i=0;i<connection;i++) {
		Surface temp;
		temp.n = surface[i].n;
...
int main(int argc, char **argv) {
	...
	flag=true;
	freopen("cow_working.ply","r",stdin);
	input();
	glutInit(&argc,argv);
